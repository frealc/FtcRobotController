...
/*
 * ============================================================================
 *  DECODEmanettePEDROBleu2  (BLEU, robot 25017)
 * ============================================================================
 *
 *  Variante BLEUE "2" de DECODEmanettePEDRO :
 ...
 * ============================================================================
 */
@TeleOp(name = "DECODEmanette PEDRO Bleu - 25017", group = "Pedro")
public class DECODEmanettePEDROBleu2 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        /* ========================================================
         *  POSITION DE TIR + CHEMIN AUTOMATIQUE (PedroPathing)
         * ========================================================
         *
        ...
        Supplier<PathChain> versTir = () -> follower.pathBuilder()
                .addPath(new Path(new BezierLine(follower.getPose(), tirePose)))
                .setLinearHeadingInterpolation(follower.getPose().getHeading(), tirePose.getHeading())
                .build();

        boolean autoTir = false;                            // etat du mode deplacement+tir automatique
        double fTirAuto = data.vitesse_tir_auto_av;         // vitesse de tir cible en mode auto (position AVANT, par defaut)
        ...
        follower.startTeleopDrive();   // initialise le mode teleop du follower
        ...
        while (opModeIsActive()) {
            ...
            // On verifie ftcPose != null : un tag detecte sans pose -> NullPointerException -> crash de l'appli.
            if (tag != null && tag.ftcPose != null) {
                fTirAuto = 2.0765 * tag.ftcPose.range + 870.63;
            }

            /* ============================================================
             *   MANETTE 1 : DEPLACEMENT AUTOMATIQUE PEDRO + TIR AUTO  (Y)
             * ============================================================ */

            // Declenchement : appui sur Y => le robot part vers la pose de tir
            if (!autoTir && manette1.y) {
                // Deplacement en PLEINE puissance (equivalent du "mode rapide" manuel = divisor 1.0),
                // pour que le trajet Pedro ne soit jamais bride.
                follower.setMaxPower(1.0);
                follower.followPath(versTir.get(), true); // true = holdEnd -> se maintient a l'arrivee
                autoTir = true;
            }

            if (autoTir) {

                // Annulation manuelle : RIGHT_BUMPER -> on rend le controle au pilote
                // (on evite dpad_down qui sert deja au monte-charge Motsoulever)
                if (manette1.right_bumper) {
                    ...
                    // Le lanceur monte en vitesse PENDANT que le robot se deplace,
                    // pour etre pret des l'arrivee.
                    roueLanceur.setVelocity(fTirAuto);
                    roueLanceur1.setVelocity(-fTirAuto);

                    // Vitesse "bonne" = meme condition que l'activation de la LED (+/- 40 tick/s)
                    boolean vitesseBonne = roueLanceur.getVelocity() > fTirAuto - 40
                            && roueLanceur.getVelocity() < fTirAuto + 40;

                    // LED (identique a la logique du mode manuel)
                    ledR.setState(vitesseBonne);
                    ledG.setState(!vitesseBonne);

                    // Le robot est arrive quand le follower a fini de suivre le chemin
                    // (le holdEnd continue de le maintenir parfaitement en place).
                    boolean arrive = !follower.isBusy();

                    // TIR AUTOMATIQUE : position atteinte ET vitesse du lanceur bonne
                    if (arrive && vitesseBonne) {
                        pousseballe.setPosition(data.servo_moteur_angle_haut); // laisse passer la balle
                        chargement_manuel.setPower(1);                         // alimente les balles
                    ...
                    // Telemetry du mode auto
                    telemetry.addLine("=== DEPLACEMENT + TIR AUTOMATIQUE ===");
                    telemetry.addData("arrive a la position", arrive);
                    telemetry.addData("vitesse bonne", vitesseBonne);
                    telemetry.addData("vitesse lanceur", roueLanceur.getVelocity());
                    telemetry.addData("vitesse cible", fTirAuto);
                    telemetry.addData("position actuelle", follower.getPose());
                    telemetry.addData("RIGHT_BUMPER pour annuler", 0);
                    telemetry.update();

                    continue; // on saute le controle manuel pendant la sequence automatique
                }
            }

            /* *******************************************
             **********************************************
             * MANETTE 1 : PILOTE DEPLACEMENT (manuel)
             **********************************************
             ********************************************** */
            ...
            if (manette1.dpad_down){
                while(opModeIsActive() && Motsoulever.getCurrentPosition() <= 100) {
            ...
            if(manette1.dpad_up){
                while(opModeIsActive() && Motsoulever.getCurrentPosition() >= 0){
            ...
            telemetry.addData("Y (manette1) = deplacement + tir automatique", 0);
