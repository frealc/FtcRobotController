...
public class testporg extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            Targetval = 1360;
            Currentval = roueLanceur.getVelocity();
            ...
            //le moteur dois allé a la vitesse max (1800 tick/s) avant de redescendre a la vitesse demandé
                ...
                while (Currentval < Targetval){
                    Currentval = roueLanceur.getVelocity();
                    erreur = Targetval - Currentval;
                    double D = (erreur - LastError)/timer.seconds();
                    IntegralSum = IntegralSum + (erreur * timer.seconds());
                    double Sortie = (Kp * erreur) + (Ki * IntegralSum) + (Kd * D);

                    roueLanceur.setPower(Sortie);
                    roueLanceur1.setPower(-Sortie);
                }
                LastError = erreur;
